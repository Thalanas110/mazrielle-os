...
fn derive_key(secret: &str, salt: &[u8]) -> Result<Zeroizing<[u8; KEY_BYTES]>, String> {
    let params = Params::new(MEMORY_SIZE, ITERATIONS, PARALLELISM, Some(KEY_BYTES))
...
fn decrypt_bytes(key: &[u8; KEY_BYTES], payload: &EncryptedPayload) -> Result<Vec<u8>, String> {
    if payload.version != CRYPTO_VERSION || payload.algorithm != "AES-256-GCM" {
        return Err("Unsupported encrypted payload".into());
    }
...
fn unwrap_key(secret: &str, wrapped: &WrappedKey) -> Result<[u8; KEY_BYTES], String> {
    ...
    let mut bytes = decrypt_bytes(&derived, &encrypted)?;
    if bytes.len() != KEY_BYTES {
        bytes.zeroize();
        return Err("Invalid vault key length".into());
    }
    let mut key = [0_u8; KEY_BYTES];
...
pub fn setup(
    master_password: &str,
    recovery_key: &str,
) -> Result<(VaultEnvelope, [u8; KEY_BYTES]), String> {
    let mut vault_key = [0_u8; KEY_BYTES];
    OsRng.fill_bytes(&mut vault_key);
    let envelope = VaultEnvelope {
        version: CRYPTO_VERSION,
        password: wrap_key(master_password, &vault_key)?,
        recovery: wrap_key(recovery_key, &vault_key)?,
    };
